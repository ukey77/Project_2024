...
class MusicPlayer {
  constructor(id) {
    ...
    // DOM_Area
    this.bodyArea = document.getElementById('bodyArea');
  ...
  updateDOM() {
    ...
    this.lyricsContTxt.textContent = currentMusic.lyricsText;
    ...
    //this.bodyArea
    this.bodyArea.style.backgroundImage = `linear-gradient(rgba(255, 239, 233, 0.9),
      rgba(192, 186, 186, 0.85)), url(${currentMusic.albumCover})`;
  ...
  musicTimeUpdate() {
    this.audioFile.addEventListener("timeupdate", () => {
      ...
      if(this.audioFile.currentTime === this.audioFile.duration && this.playBtn.style.display == "none"){
        if (this.repeatBtn.classList.contains("xi-repeat-one")) {
          // repeat
  ...
  backwardPlay() {
    this.backwardBtn.addEventListener("click", () => {
      ...
      if(this.audioFile.currentTime > 2){
        this.moveToCurrentTrack();
        this.musicPlayBtnFunc();
      }else{
  ...
  moveToRandomTrack() {
    if (this.shuffleBtn.classList.contains("shuffleActiveBtn")) {
        let previousNum = this.currentLink ? this.currentLink.id : -1;
        let randomNum = previousNum;

        for (let i = 0; i < this.musicLength; i++) {
            randomNum = Math.floor(Math.random() * this.musicLength);
            if (randomNum !== previousNum) {
                break;
            }
        }

        this.currentLink = musicNodeLink.musicLinks[randomNum];
        this.currentNode = musicContents[this.currentLink.id];
        this.updateDOM();
        this.musicPlayBtnFunc();
    } else {
...
//
  ...
  repeatMusicPlay() {
    this.repeatBtn.addEventListener("click", () => {
      ...
      if(!this.repeatBtn.classList.contains("repeatActiveBtn")){
        this.repeatBtn.classList.add("repeatActiveBtn");
      }else if(
        this.repeatBtn.classList.contains("repeatActiveBtn") && 
        !this.repeatBtn.classList.contains("xi-repeat-one")
      ){
        this.repeatBtn.classList.add("xi-repeat-one");
      }else if(
        this.repeatBtn.classList.contains("xi-repeat-one")&&
        this.repeatBtn.classList.contains("repeatActiveBtn")
      ){
        this.repeatBtn.classList.remove("xi-repeat-one")
  ...
  shuffleMusicPlay() {
    this.shuffleBtn.addEventListener("click", () => {
      ...
      // 나머지제거
      if(this.shuffleBtn.classList.contains("shuffleActiveBtn")){
        this.repeatBtn.classList.remove("repeatOneActiveBtn");
        this.repeatBtn.classList.remove("repeatActiveBtn");
        this.repeatBtn.classList.remove("xi-repeat-one");
      }
    })
